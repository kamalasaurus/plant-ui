...
module PlantSampleUpload
  ...
  def self.parse_freeze_dried(file)
    col_sep = (file =~ /\t/ ? "\t" : ',')
    CSV.parse(file, headers: true, header_converters: %i[downcase symbol], col_sep:) do |row|
      h = row.to_h
      ...
      attrs[:species_id] = Species.find_by(genus:, species:).id
      attrs = attrs.except(:species)
      ...
      ActiveRecord::Base.transaction do
        ...
        full_attrs = attrs.merge({ accession_id: })
        ...
        if seeds.length.zero?
          puts 'NO SEEDS'
          puts SeedsPlantSample.where(plant_sample_id:).map(&:id).join(' ')
  ...
  def self.parse_silica(file)
    ...
    CSV.parse(file, headers: true, header_converters: %i[downcase symbol], col_sep:) do |row|
      ...
      attrs = h
              ...
              .transform_values { |v| %w[Placeholder NA].include?(v) ? nil : v }
              .tap do |hash|
